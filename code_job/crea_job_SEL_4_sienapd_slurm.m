...
filename_qsub=fullfile(base_directory,'code','Jobs',['SEL_sbatch_4_sienapd_slurm_' datestr(now,30) '.txt']);
ID_file_recap=fopen(filename_qsub,'w');

jobdir=fullfile(base_directory,'code','Jobs','Exex_slurm_sienapd_SEL');
if not(exist(jobdir,'dir'))
    mkdir(jobdir)
end
    ...
    subj_data_list_follow=dir(subj_path_data_follow);
        ...
        subj_output_list=dir(subj_path_output);
                ...
                fprintf(ID_file,['addpath(''/nfsd/biopetmri/BACKUP/Users/Marco/3T_Verona/SEL_candidates/code/NIFTI_toolbox/'')\n'...
                    ...%'addpath /nfsd/biopetmri/BACKUP/Users/Marco/3T_Verona/SEL_candidates/SEL/code_matlab/registration_sienapd'
                    'nak_detJ_FLAIR = load_untouch_nii(''' strrep(fullfile(subj_path_output,subj_output_list(3).name,'registration_sienapd','FLAIR_T1','Jacobian','nak_detJ_FLAIR.nii.gz'),'\','/') ''')' ';' '\n'...
                    'lesions_prob_base_halfway = load_untouch_nii(''' strrep(fullfile(subj_path_output,subj_output_list(3).name,'registration_sienapd','FLAIR_T1','Jacobian','lesion_prob_base_halfway_ANTs.nii.gz'),'\','/') ''')' ';' '\n'...
                    'lesions_prob_foll_halfway = load_untouch_nii(''' strrep(fullfile(subj_path_output,subj_output_list(3).name,'registration_sienapd','FLAIR_T1','Jacobian','lesion_prob_foll_halfway_ANTs.nii.gz'),'\','/') ''')' ';' '\n'...
                    ...
                    'save_untouch_nii(nifti_SEL_' num2str(str2double(subjID_output)) ',''' strrep(fullfile(subj_path_output,subj_output_list(3).name,'registration_sienapd','FLAIR_T1','Jacobian','SEL.nii'),'\','/') ''')' ';' '\n'...
                    '\n'...
                    '\n'...
                    'nak_detJ_T2 = load_untouch_nii(''' strrep(fullfile(subj_path_output,subj_output_list(3).name,'registration_sienapd','T2_T1','Jacobian','nak_detJ_T2.nii.gz'),'\','/') ''')' ';' '\n'...
                    ...
                    'nifti_SEL_' num2str(str2double(subjID_output)) ' = lesions_prob_base_halfway;' '\n'...
                    'nifti_SEL_' num2str(str2double(subjID_output)) '.img = SEL_' num2str(str2double(subjID_output)) ';' '\n'...
                    'save_untouch_nii(nifti_SEL_' num2str(str2double(subjID_output)) ',''' strrep(fullfile(subj_path_output,subj_output_list(3).name,'registration_sienapd','T2_T1','Jacobian','SEL.nii'),'\','/') ''')' ';' '\n' ...
